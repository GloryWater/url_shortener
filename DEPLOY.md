...
> 💡 **Новое:** Полная документация по CI/CD доступна в [`CI_CD.md`](CI_CD.md)
...
> 📖 **Полная документация:** См. [`CI_CD.md`](CI_CD.md) для детального руководства по CI/CD.

GitHub Actions автоматически развернет приложение при пуше в `main`.

### Workflow файлы

Проект использует **4 workflow** для автоматизации:

| Workflow | Описание | Триггеры |
|----------|----------|----------|
| **CI** (`.github/workflows/ci.yaml`) | Тесты, линтеры, безопасность | Push в main/develop, PR |
| **CD** (`.github/workflows/cd.yaml`) | Сборка и деплой | Push в main |
| **Manual Deploy** (`.github/workflows/manual-deploy.yaml`) | Ручной деплой | Workflow dispatch |
| **Release** (`.github/workflows/release.yaml`) | Релизы с тегами | Push тега v* |
